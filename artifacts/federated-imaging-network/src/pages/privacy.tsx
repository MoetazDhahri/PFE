        ...
        <div className="space-y-5">
          ...
          <div className="grid gap-5 xl:grid-cols-[.85fr_1.15fr]">
            <Panel label="Privacy accountant" title="Budget posture">
              <div className="p-5 sm:p-6"><div className="flex items-end justify-between"><div><p className="font-display text-4xl font-semibold tracking-[-.07em]">{overview.data?.privacyBudget.toFixed(1) ?? '—'}<span className="ml-1 text-lg text-muted-foreground">ε</span></p><p className="mt-2 text-xs text-muted-foreground">Epsilon budget remaining for the active track</p></div><StatusChip value={overview.data?.privacyStatus} kind="good" /></div><div className="mt-7 h-2 rounded-full bg-muted"><div className="h-full w-[34%] rounded-full bg-primary" /></div><div className="mt-2 flex justify-between font-mono text-[9px] uppercase tracking-[.1em] text-muted-foreground"><span>0 consumed</span><span>policy ceiling 8.0 ε</span></div><div className="mt-7 grid gap-3 text-xs"><PolicyRow label="Gradient clipping" value="Enabled" /><PolicyRow label="Noise mechanism" value="Gaussian / σ 1.2" /><PolicyRow label="Secure aggregation" value="Required" /><PolicyRow label="Raw DICOM egress" value="Blocked" /></div></div>
            </Panel>
