...
- `ml/federation-engine` — standalone Python FL simulation (Flower/PyTorch), isolated from the pnpm workspace and CI; intended to eventually feed real training results into the DB via `lib/db/src/import-training-run.ts`, replacing the current seeded numbers.
...
- **Simulated vs. real is tracked explicitly**: node metrics and round numbers are currently seeded/simulated arithmetic, not output of real training. Real FL exists only as the disconnected `ml/federation-engine` Python service. This boundary is deliberately visible to users via the Academy's per-concept status tags, not hidden.
...
- **Federation Agent assessments**: an LLM observes aggregate telemetry (never raw imaging data) and produces an "Action Value Card" recommending an action (e.g., hold a node's update before aggregation); a human approves or dismisses it.
...
- `ml/federation-engine` is not part of the pnpm workspace, typecheck, or build pipeline — it won't be caught by `pnpm run typecheck`/`build` and currently has no automated bridge into the Node/Postgres stack.
- `agent.ts`'s `observedValueFor`/`outcomeFor` are template strings today, not a second real model call — don't assume assessment text reflects live computation beyond what `submit_assessment` returned.
